...
pub fn get_rand_bytes(num_bytes: u8) [num_bytes]u8 {
    var values = @Vector(num_bytes, u8);
    for (0..num_bytes) |i| {
        values[i] = std.crypto.random.intRangeAtMost(u8, 0, 255);
    }
    return values;
}

fn get_rand_byte() u8 {
    const value = get_rand_bytes(1)[0];
    return value;
}

fn get_nonzero_rand_byte() u8 {
    while (true) {
...
fn new_coefficients(intercept: u8, degree: u8) [degree + 1]u8 {
    const coefficients = [degree + 1]u8{};
    ...
    for (0..degree) |i| {
        ...
        const coeff_idx = i + 1;
        coefficients[coeff_idx] = if (coeff_idx == degree) get_nonzero_rand_byte() else get_rand_byte();
    }
    return coefficients;
}
...
fn new_coordinates() [255]u8 {
    const coordinates = [255]u8{};
    for (0..255) |i| {
        coordinates[i] = i + 1;
    }
    ...
    const random_indices: [255]u8 = get_rand_bytes(255);
    for (0..255) |i| {
        const j: u8 = random_indices[i] % 255; // Make sure to handle the case where the byte is 255.
...
pub fn gdiv(a: u8, b: u8) DivisionError!u8 {
    ...
    const diff: u8 = (logA - logB + 255) % 255;
...
fn evaluate(coefficients: []u8, x: u8, degree: u8) EvaluationError!u8 {
    if (x == 0) {
...
pub fn split(
    ...
    var result = [secret.len]u8{};
    const secret_len = secret.len;
    const x_coordinates = new_coordinates();

    // Add tags to identify each share
    for (0..shares) |k| {
        const share = [secret_len + 1]u8{};
        share[secret_len] = x_coordinates[k];
        result[k] = share;
    }
    ...
    for (0..secret_len) |i| {
        ...
        const coeffs = new_coefficients(secret_byte, degree);
        for (0..shares) |k| {
...
pub fn combine(shares: [][]u8) []u8 {
    ...
    const share_len = shares[0].len;
    // This will be our reconstructed secret
    // Note: secret is 1 byte smaller than a share
    // since share includes an extra identifier tag
    const secret_len = share_len - 1;
    const secret = [secret_len]u8{};
    // const secretLength = shareLength - 1;
    // const secret = new Uint8Array(secretLength);

    const x_samples = [share_len]u8{};
    const y_samples = [share_len]u8{};
